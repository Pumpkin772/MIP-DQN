...
class Actor_MIP:
    ...
    def predict_best_action(self, state):
        ...
        if self.constrain_on:
            ...
            m.power_con17 = pyo.Constraint(expr=(
                    m.nn.inputs[8] * self.scaled_parameters[1] + m.nn.inputs[4] * self.scaled_parameters[5] >= \
                    self.env.dg1.power_output_min))
            m.power_con18 = pyo.Constraint(expr=(
                    m.nn.inputs[8] * self.scaled_parameters[1] + m.nn.inputs[4] * self.scaled_parameters[5] <= \
                    self.env.dg1.power_output_max))
            m.power_con19 = pyo.Constraint(expr=(
                    m.nn.inputs[9] * self.scaled_parameters[2] + m.nn.inputs[5] * self.scaled_parameters[6] >= \
                    self.env.dg1.power_output_min))
            m.power_con20 = pyo.Constraint(expr=(
                    m.nn.inputs[9] * self.scaled_parameters[2] + m.nn.inputs[5] * self.scaled_parameters[6] <= \
                    self.env.dg1.power_output_max))
            m.power_con21 = pyo.Constraint(expr=(
                    m.nn.inputs[10] * self.scaled_parameters[3] + m.nn.inputs[6] * self.scaled_parameters[7] >= \
                    self.env.dg1.power_output_min))
            m.power_con22 = pyo.Constraint(expr=(
                    m.nn.inputs[10] * self.scaled_parameters[3] + m.nn.inputs[6] * self.scaled_parameters[7] <= \
                    self.env.dg1.power_output_max))
            m.battery_con23 = pyo.Constraint(expr=(
                    (m.nn.inputs[2] * self.env.battery.capacity + m.nn.inputs[7] * self.scaled_parameters[
                        0]) / self.env.battery.capacity <= \
                    self.env.battery.max_soc))
            m.battery_con24 = pyo.Constraint(expr=(
                    (m.nn.inputs[2] * self.env.battery.capacity + m.nn.inputs[7] * self.scaled_parameters[
                        0]) / self.env.battery.capacity >= \
                    self.env.battery.min_soc))
